#!/usr/bin/env bash
# -----------------------------------------------------------------------------
#  gitflow-init.sh  –  Zero-config semantic-release, commitlint & Husky
#  Created by arizmuajianisan
#  v1.0.0  –  Safe for CI or humans
# -----------------------------------------------------------------------------
set -euo pipefail

SCRIPT_NAME="gitflow-init"
VERSION="1.0.0"
AUTHOR="arizmuajianisan"
RELEASE_URL="https://github.com/arizmuajianisan/gitflow-init/releases/download/v${VERSION}/gitflow-init.sh"
...
cat > .release-it.json <<'EOF'
{
  ...
  "git": { "commitMessage": "chore(release): ${version}", "tagName": "v${version}", "requireBranch": "main" },
  "github": { "release": true, "releaseName": "v${version}", "tokenRef": "GITHUB_TOKEN" },
  "npm": { "publish": false },
  "plugins": {
    "@release-it/conventional-changelog": {
